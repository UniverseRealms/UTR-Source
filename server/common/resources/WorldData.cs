...
namespace common.resources
{
    ...
    {
        private static readonly ILog Log = LogManager.GetLogger(typeof(WorldData));
            ...
            for (var i = 0; i < jwFiles.Length; i++)
            {
                ...
                for (var j = 0; j < world.maps.Length; j++)
                {
                    ...
                    else
                    {
                        ...
                        try { world.wmap[j] = Json2Wmap.Convert(gameData, jm); }
                        catch (Exception e) { Log.Error($"[World Data Exception]:\n- Path: {mapFile};\n- Exception: {e.ToString()}"); }
                    }
                }
                worlds.Add(world.name, world);
            }
