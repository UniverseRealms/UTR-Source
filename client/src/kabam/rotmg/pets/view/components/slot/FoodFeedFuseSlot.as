...
public class FoodFeedFuseSlot extends FeedFuseSlot {
    ...
    private function endDrag(_arg1:MouseEvent):void {
        ...
        addChild(itemSprite);
        alignBitmapInBox();
        var _local2:* = DisplayHierarchy.getParentWithTypeArray(itemSprite.dropTarget, ModalItemSlot);
        if (!(((_local2 is ModalItemSlot) && (_local2 as ModalItemSlot).interactable == true))) {
            this.empty = true;
            itemId = ItemConstants.NO_ITEM;
            itemBitmap.bitmapData = null;
            this.clearAndCallCancel();
