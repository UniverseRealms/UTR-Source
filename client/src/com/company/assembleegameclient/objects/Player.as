...
public class Player extends Character {
    ...
    public var skillPoints_:int = 0;
    public var usedSkillPoints_:int = 0;
    public var boughtSkills_:Vector.<SkillTreeData> = new Vector.<SkillTreeData>();
    ...
    private function doShoot(time:int, weaponType:int, weaponXML:XML, attackAngle:Number, useMult:Boolean):void {
        ...
        while (_local10 < numProjs) {
            ...
            dmgMin = int(proj.projProps_.minDamage_);
            dmgMax = int(proj.projProps_.maxDamage_);
            dmgMulti = useMult ? this.attackMultiplier() : 1;
            ...
            if(this.boughtSkills_[1] != null && !useMult && this.boughtSkills_[1].unlockedSkillsId_ == 1) //Gold Node #1 25% + dmg to abilities
               dmg *= 1.25;

            if(this.boughtSkills_[2] != null && useMult && this.boughtSkills_[2].unlockedSkillsId_ == 1) //Gold Node #2 25% + dmg to projs
               dmg *= 1.25;

            if (time > (map_.gs_.moveRecords_.lastClearTime_ + 600))
                dmg = 0;
            proj.setDamage(dmg, isCrit, CriticalMulti());
            if ((((_local10 == 0)) && (!((proj.sound_ == null)))))
