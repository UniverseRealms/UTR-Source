    ...
    public class NetworkTicker
    {
        ...
        public void TickLoop()
        {
            foreach (var pending in Pendings.GetConsumingEnumerable())
            {
                ...
                if (pending.Item1.State == ProtocolState.Disconnected || pending.Item1.Id != pending.Item2)
                    continue;
                ...
                {
                    Program.Debug(typeof(NetworkTicker), $"Error processing packet ({pending.Item1.Account?.Name ?? "null"}, {pending.Item1.IP}, {pending.Item2})\n{e.ToString()}", true);

                    pending.Item1.SendFailureDialog("Network Read Error", "An error occurred while processing data from your client.");
                }
            }
        }
