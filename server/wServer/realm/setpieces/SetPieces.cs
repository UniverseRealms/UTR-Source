...
namespace wServer.realm.setpieces
{
    partial class SetPieces
    {
        ...
        private static readonly List<Tuple<ISetPiece, int, int, WmapTerrain[]>> setPieces = new List<Tuple<ISetPiece, int, int, WmapTerrain[]>>()
        {
			...
			//SetPiece(new LavaFissure(), 1, 1, WmapTerrain.Mountains)// SpecialDay:30 )
		...
		public static void ApplySetPieces(World world)
		{
			...
			foreach (Tuple<ISetPiece, int, int, WmapTerrain[]> dat in setPieces)
			{
                ...
                // TODO: add day specific attribute
				if (true)
				{
			...
			Program.Debug(typeof(SetPieces), "It is currently day { " + today.Day + " } The Frog Event happens every month on the {30th}[ Please do not spawn this Set Piece.]");
			Program.Debug(typeof(SetPieces), "SetPiece Applied.");
		}
