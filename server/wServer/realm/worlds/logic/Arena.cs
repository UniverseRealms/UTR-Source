...
namespace wServer.realm.worlds.logic
{
    internal class Arena : World
    {
        ...
        public override World GetInstance(Client client)
        {
            ...
            {
                if (!(world is Arena) || world.Limbo || (world as Arena)._solo)
                    continue;
            ...
            var arena = new Arena(manager.Resources.Worlds[Name], client) { Limbo = false };
        ...
        public override void Tick()
        {
            ...
            if (Limbo || Deleted || TotalConnects < 1)
            {
                if (!Deleted || _solo || Manager == null)
                    return;

                Manager.Monitor.OpenPortal(Arena);
                Manager.Monitor.UpdateWorldInstance(Arena, Manager.Worlds[Arena]);
                return;
            }
        ...
        private void HandleWaveRewards()
        {
            ...
            {
                if (!gameData.IdToObjectType.TryGetValue(reward, out ushort itemType))
                    continue;

                if (!gameData.Items.TryGetValue(itemType, out Item item))
                    continue;
