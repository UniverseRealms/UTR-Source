...
namespace wServer.realm.terrain
{
    ...
    public class WmapTile
    {
        ...
        {
            // returns a clone of the tile from the original map.
            // If you change a tile after the map is loaded (i.e. from a behavior)
            // then this will return the tile that existed before the tile was changed
    ...
    public class Wmap
    {
        ...
        private (int coordX, int coordY) ValidateWmapCoordinate(int newX, int newY)
        {
            if (newX < 0) newX = 0;
            if (newY < 0) newY = 0;
        ...
        public WmapTile this[int x, int y]
        {
            get
            {
                var (coordX, coordY) = ValidateWmapCoordinate(x, y);

                return _tiles[coordX, coordY];
            }
            set
            {
                var (coordX, coordY) = ValidateWmapCoordinate(x, y);

                _tiles[coordX, coordY] = value;
            }
        }
            ...
            using (var rdr = new BinaryReader(new ZlibStream(stream, CompressionMode.Decompress)))
            {
                ...
                for (var i = 0; i < c; i++)
                {
                    ...
                    if (_dat.IdToObjectType.ContainsKey(obj))
                        ...
                        Program.Debug(typeof(Wmap), $"Object: {obj} not found.", warn: true);
