...
public class GameServerConnectionConcrete extends GameServerConnection {
    ...
    override public function connect():void {
        ...
        serverConnection.connect(server_.address, 2050);
    }
    ...
    public function mapMessages():void {
        ...
        _local1.map(BUY_SKILL).toMessage(BuySkill);
        _local1.map(UPDATE_TREE).toMessage(UpdateTree);
        _local1.map(SKILL_TREE).toMessage(SkillTreeResult).toMethod(this.onSkillTreeResult);
    }
    ...
    private function onSkillTreeResult(skillTreeResult:SkillTreeResult): void {

        if(this.gs_.map.player_ == null)
        {
            this.player.boughtSkills_ = skillTreeResult.results_;
        }
        else {
            this.gs_.map.player_.boughtSkills_ = skillTreeResult.results_;
        }
        SkillTreeSignal.instance.dispatch(skillTreeResult);
    }
    ...
    private function onDamage(_arg1:Damage):void {
        ...
        //var _local5:int;
        var map:Map = gs_.map;
        var target:GameObject = map.goDict_[_arg1.targetId_];
        if (target != null) {
            target.damage(-1, _arg1.damageAmount_, _arg1.effects_, _arg1.kill_, null);
        }
    }
        ...
        for each (statData in statDataVec) {
            ...
            switch (statData.statType_) {
                ...
                case StatData.M_POTIONS:
                    ...
                    break;
                case StatData.SET_SKILL_POINTS:
                    player.skillPoints_ = statValue;
                    break;
                case StatData.SET_USED_SKILL_POINTS:
                    player.usedSkillPoints_ = statValue;
                    break;
                case StatData.BOUGHT_SKILLS:
                    //player.boughtSkills_ = Vector.<int>(statValue);
                    break;
            }
        }
    }
    ...
    private function onReconnect(_arg1:Reconnect):void {
        var _local2:Server = new Server().setName(_arg1.name_).setAddress((((_arg1.host_) != "") ? _arg1.host_ : server_.address)).setPort(2050);
