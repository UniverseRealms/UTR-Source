...
namespace wServer.realm
{
    public class ConInfo : IEquatable<ConInfo>
    {
        ...
        public bool Equals(ConInfo other) => GUID.Equals(other.GUID);
    ...
    internal class ConnectionQueue
    {
        ...
        public void KeepAlive()
        {
            for (var i = _queue.Count - 1; i >= 0; i--)
            {
                ...
                if (conInfo.Client != null && conInfo.Client.KeepAlive(i + 1, Count)) continue;

                _queue.RemoveAt(i);
            }
        }

        public int Position(ConInfo conInfo) => _queue.IndexOfKey(conInfo) + 1;
    }
}
