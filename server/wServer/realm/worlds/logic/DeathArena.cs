...
{
    internal class DeathArena : World
    {
        ...
        {
            { 5,  new[] { "Onrane Cache", "1000 Gold" } },
            { 10, new[] { "1000 Gold", "Onrane Cache", "Medium Sor Fragment" } },
            { 20, new[] { "Ultimate Onrane Cache", "Medium Sor Fragment" } },
            { 25, new[] { "1000 Gold", "Onrane Cache", "Gold Cache", "Medium Sor Fragments" } },
            { 30, new[] { "1000 Gold", "10000 Gold", "Onrane Cache", "Two Tiny Sor Fragments" } },
            ...
            : base(proto)
        {
            Instance = this;
            _isOpen = true;
            WorldLoot.Add(new ItemLoot("Champion's Arena Key", 0));
        ...
        public override World GetInstance(Client client)
        {
            ...
            {
                if (world.Limbo || !(world is DeathArena da) || da.CurrentState == ArenaState.Ended)
                    continue;
            ...
            var arena = Manager.AddWorld(
                new DeathArena(manager.Resources.Worlds[Name], client) { Limbo = false });
        ...
        public override void Tick()
        {
            ...
            _time += (int)CoreConstant.worldTickMs;
        ...
        private void Countdown()
        {
            if (_countDown == CountDownState.Notify60)
            {
                ...
                foreach (var w in Manager.Worlds.Values)
                    foreach (var p in w.Players.Values)
                        ...
                        {
                            p.SendInfo($"<{p.Manager.Config.serverInfo.name}> Champion's Arena closing in 1 minute. Head to the Nexus to join!");
                            if (p.Owner is Nexus || p.Owner is Vault)
                            {
                                p.Client.SendPacket(new GlobalNotification
                                {
                                    Type = GlobalNotification.ADD_ARENA,
                                    Text = "{\"name\":\"Champion's Arena\",\"open\":true}"
                                });
                            }
                        }
            }
                ...
                foreach (var w in Manager.Worlds.Values)
                    foreach (var p in w.Players.Values)
                        p.SendInfo(p.Owner == this
                            ...
                            : $"<{p.Manager.Config.serverInfo.name}> Champion's Arena closing in 30 seconds. Head to the Nexus to join!");
            }
            ...
            if (_countDown == CountDownState.StartGame && _time > 60000)
            {
                ...
                //    {
                //        Type = GlobalNotification.ADD_ARENA,
                //        Text = "{\"name\":\"Champion's Arena Arena\",\"open\":false}"
                //    });
            }
        }

        private void WaitForPlayersToLeave()
        {
            if (Players.Count == 0)
            {
        ...
        private void Rest(bool recover = false)
        {
            if (recover)
            {
        ...
        private void HandleWaveRewards()
        {
            ...
            foreach (var player in Players.Values.Where(p => !p.HasConditionEffect(ConditionEffects.Hidden)))  // no rewards for lurkers
            {
                ...
                if (slot != -1)
                {
                    ...
                    player.SendInfo($"[Champion's Arena] You have been granted a reward: '{item.ObjectId}' (wave {_wave})");
                }
                else
                {
                    player.SendError("[Champion's Arena] We were unable to give you a reward, your inventory is full.");
                }
            }
        }
        ...
        private void LockPortal()
        {
            ...
            var en = Entity.Resolve(Manager, "Locked Champion's Arena Portal");
            en.Move(79f, 50f);
            en.Name = "Champion's Arena";
            nexus.EnterWorld(en);
        }
        ...
        public override int EnterWorld(Entity entity)
        {
            ...
            if (entity is Player p)
                p.SendInfo("Welcome to Champion's Arena. Take caution, you can die here.");
