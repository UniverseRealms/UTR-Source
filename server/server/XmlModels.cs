...
namespace server
{
    ...
    class PartyMember
    {
        ...
        public static PartyMember FromDb(DbAccount acc)
        {
    ...
    class Party
    {
        ...
        public static Party FromDb(Database db, DbParty party)
        {
            var members = (from member in party.Members
                           select db.GetAccount(member) into acc
                           where acc != null
                           orderby acc.PartyRank descending,
                                   acc.Name ascending
                           select PartyMember.FromDb(acc)).ToList();
        ...
        public XElement ToXml()
        {
            ...
            party.Add(new XAttribute("id", _id));
            foreach (var member in _members)
                party.Add(member.ToXml());
    ...
    class PartyIdentity
    {
        ...
        public static PartyIdentity FromDb(DbAccount acc, DbParty party)
        {
    ...
    internal class Account
    {
        ...
        public PartyIdentity Party { get; private set; }
        ...
        public static Account FromDb(DbAccount acc)
        {
            return new Account()
            {
                ...
                Party = PartyIdentity.FromDb(acc, new DbParty(acc)),
    ...
    internal class Character
    {
		...
		public static Character FromDb(DbChar character, bool dead)
        {
			return new Character()
			{
				...
				Luck = character.Stats[8],
				Restoration = character.Stats[9],
    ...
    internal class ClassAvailability
    {
        ...
        public static ClassAvailability FromDb(Database db, DbAccount acc)
        {
            ...
            foreach (string c in cs.AllKeys
                .Select(key => _classes[Convert.ToUInt16(key)]))
