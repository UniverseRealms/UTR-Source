...
public class Projectile extends BasicObject {
    ...
    public var crit_:Boolean;
    public var multi_:Number;
    ...
    public function setDamage(dmg:int, crit:Boolean=false, multi:Number=0):void {
        this.damage_ = dmg;
        this.crit_ = crit;
        this.multi_ = multi;
    }
    ...
    override public function update(currentTime:int, msDelta:int):Boolean {
        ...
        if (go != null) {
            ...
            var goIsAlly:Boolean = go.props_.isAlly_;
            var goIsPlayer:Boolean = go.props_.isPlayer_;
            var goHit:Boolean = player != null &&
                    !player.isPaused() &&
                    !player.isHidden() &&
                    (this.damagesPlayers_ || goIsEnemy && this.ownerId_ == player.objectId_ || this.damagesFromPlayer_);

            if (goHit) {
                var dmg:int = GameObject.damageWithDefense(this.damage_, go.defense_, this.projProps_.armorPiercing_, go.condition_);
                var dmgEnemy:int = GameObject.damageWithDefenseEnemy(this.damage_, go.defense_, this.projProps_.armorPiercing_, go.condition_);
                ...
                if (go == player) {
                    ...
                    if (goIsEnemy) {
                        ...
                        if(this.crit_)
                            go.criticalDamage(this.containerType_, dmgEnemy, this.projProps_.effects_, killed, this, this.multi_);
                        else
                            go.damage(this.containerType_, dmgEnemy, this.projProps_.effects_, killed, this);
                    }
                    else if (goIsAlly) {
                        map_.gs_.gsc_.allyHit(currentTime, this.bulletId_, go.objectId_, this.ownerId_);
    ...
    public function getHit(pX:Number, pY:Number) : GameObject {
        ...
        if (damagesEnemies_) {
            for each(go in map_.hitTEnemies_) {
                ...
                var circleDiste:Number = Math.pow(Math.pow(xDiff, 2) + Math.pow(yDiff, 2), 0.5);
                if (!(circleDiste > go.radiusenemy_)) {
            ...
            for each(go in map_.hitTPlayers_) {
                xDiff = go.x_ > pX ? Number(go.x_ - pX) : Number(pX - go.x_);
                yDiff = go.y_ > pY ? Number(go.y_ - pY) : Number(pY - go.y_);
                var circleDist:Number = Math.pow(Math.pow(xDiff, 2) + Math.pow(yDiff, 2), 0.5);
                if (!(circleDist > go.radiusplayer_)) {
                    if (!(this.projProps_.multiHit_ && this.multiHitDict_[go] != null)) {
                        if (go == map_.player_) {
                            return go;
                        }
    ...
    override public function draw(gfxVec:Vector.<IGraphicsData>, camera:Camera, time:int):void {
        ...
        var _loc11_:Number = !_loc10_?Number(Number(this.angle_)):Number(Number(this.getDirectionAngle(time)));
        var _loc12_:Number = !this.projProps_.noRotation_?Number(Number(_loc11_ - camera.angleRad_ + this.props_.angleCorrection_ + rotation)):Number(Number(camera.angleRad_ + this.props_.angleCorrection_));
        this.p_.draw(gfxVec,this.staticVector3D_,_loc12_,camera.wToS_,camera,tex);
